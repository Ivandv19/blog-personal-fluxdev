...
const checkRateLimit = async (kv: KVNamespace, ip: string): Promise<boolean> => {
    ...
    const data = (await kv.get(key, "json")) as { attempts: number; resetAt: number } | null;

    // Caso 1: Primer intento o ventana expirada → resetear contador
    if (!data || now > data.resetAt) {
        await kv.put(key, JSON.stringify({ attempts: 1, resetAt: now + windowMs }), { expirationTtl: 900 });
        return true;
    }
    ...
    await kv.put(key, JSON.stringify({ attempts: data.attempts + 1, resetAt: data.resetAt }), {
...
app.get("/api/comments/:slug", validateSlug, async (c) => {
    ...
    try {
        ...
        await c.env.KV.put(`comments:${slug}`, JSON.stringify(results), { expirationTtl: 300 });
