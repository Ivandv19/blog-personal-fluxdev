...
export async function POST({ params, request, locals }: APIContext) {
  ...
  try {
    ...
    if (!author || !content) {
      return new Response(JSON.stringify({ error: 'Author and content required' }), { status: 400 });
    }
    ...
    const { success } = await locals.runtime.env.DB
      .prepare('INSERT INTO comments (post_slug, author, content) VALUES (?, ?, ?)')
